            ...
            <div class="col-xl-12">
                <div class="card">
                    <div class="card-header">
                        <h5>Pay As You Go Course Report Detail</h5>
...
@php
    $shop = DB::table('shop_cart_items')->where('shop_type','paygo-course')->where('product_id',$id)->where('type','order')->get();
...
@php
    ...
    $shop = DB::table('shop_cart_items')->where('shop_type','paygo-course')->where('product_id',$id)->where('type','order')->get();
    $participants = \App\PayGoCourseBookedDate::where('course_id', $course->id)->get();
    $total_revenue = [];
    $male_user = [];
    $female_user = [];
@endphp
...
@php
    $male_user[] = DB::table('users')->where('id',$par->child_id)->where('gender','male')->count();
    $female_user[] = DB::table('users')->where('id',$par->child_id)->where('gender','female')->count();
@endphp
@endforeach
@foreach($shop as $bca)
@php
