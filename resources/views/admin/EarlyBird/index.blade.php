                                        ...
                                        <div class="table-responsive">
                                          @include('admin.error_message')
                                            <table class="table table-hover">
                                                ...
                                                <tbody>
                                                @foreach($EarlyBird as $test)
                                                    <tr>
                                                        <td>{{date('d-m-Y',strtotime($test->early_bird_date))}}</td>
                                                        <td>{{$test->early_bird_time}}</td>
                                                        <td>{{$test->discount_percentage}}</td>
                                                        @php 
                                                            $EarlyBirdLinkedCategoryName = \App\LinkCourseAndCategory::where('id',$test->course_category_id)->first();
                                                        @endphp
                                                        <td>{{ !empty($EarlyBirdLinkedCategoryName) ? $EarlyBirdLinkedCategoryName->title : '--' }}</td>
                                                        <td>
                                                        @if($test->early_bird_option == '1')
                                                        ...
                                                        <td>
                                                            <div class="btn-group">
                                                                ...
                                                                <div class="dropdown-menu" role="menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(82px, -64px, 0px); top: 0px; left: 0px; will-change: transform;">
                                                                    <a href="{{ route('admin.edit.early_bird',$test->id) }}" class="dropdown-item">Edit</a>
                                                                    <a href="{{route('admin.status.early_bird',$test->id)}}" class="dropdown-item">
                                                                        @if($test->early_bird_option == '1')
                                                                            In-active
                                                                        @else
                                                                            Active
