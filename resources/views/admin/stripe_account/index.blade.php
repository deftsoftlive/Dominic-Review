                    ...
                    <div class="table-responsive">
                      @include('admin.error_message')
                        <table class="table table-hover">
                            ...
                            <tbody>
                            @foreach($stripe_account as $test)
                                <tr>
                                    <td>{{$test->account_name}}</td>
                                    <td>{{$test->acc_holder_name}}</td>
                                    <td>
                                    @if($test->status == '1')
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(82px, -64px, 0px); top: 0px; left: 0px; will-change: transform;">
                                                <a href="{{url('admin/stripe_account')}}/{{$test->slug}}" class="dropdown-item">Edit</a>

                                                <a href="{{route('admin.stripe_account.status',$test->slug)}}" class="dropdown-item">
                                                    @if($test->status == '1')
                                                        In-active
                                                    @else
                                                        Active
                                                ...
                                                <a onclick="return confirm('Are you sure you want to delete this stripe account?')" href="{{url('admin/stripe_account/delete')}}/{{$test->id}}" class="dropdown-item">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
