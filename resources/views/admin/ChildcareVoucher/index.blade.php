                                        ...
                                        <div class="table-responsive">
                                          @include('admin.error_message')
                                            <table class="table table-hover">
                                                ...
                                                <tbody>
                                                @foreach($ChildcareVoucher as $test)
                                                    <tr>
                                                        <td>{{$test->provider_name}}</td>
                                                        <td>{{$test->provider_code}}</td> 
                                                        <td>
                                                        @if($test->status == '1')
                                                        ...
                                                        <td>
                                                            <div class="btn-group">
                                                                ...
                                                                <div class="dropdown-menu" role="menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(82px, -64px, 0px); top: 0px; left: 0px; will-change: transform;">
                                                                    <a href="{{url('admin/childcare-vouchure')}}/{{$test->slug}}" class="dropdown-item">Edit</a>

                                                                    <a href="{{route('admin.ChildcareVoucher.status',$test->slug)}}" class="dropdown-item">
                                                                        @if($test->status == '1')
                                                                            In-active
                                                                        @else
                                                                            Active
