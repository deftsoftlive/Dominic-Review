                    ...
                    <div class="table-responsive">
                      @include('admin.error_message')
                        <table class="table table-hover">
                            ...
                            <tbody>
                            @foreach($HomeSlider as $test)
                                <tr>
                                    <td><input type="text" id="update_homeslider_sort" data-id="{{$test->id}}" value="{{$test->sort}}" style="width:
                                                        50px"></td>
                                    <td>{{$test->title}}</td>
                                    <td>{{$test->heading}}</td>
                                    <td>
                                    ...
                                    <td>
                                        <div class="btn-group">
                                            ...
                                            <div class="dropdown-menu" role="menu" x-placement="bottom-start" style="position: absolute; transform: translate3d(82px, -64px, 0px); top: 0px; left: 0px; will-change: transform;">
                                                <a href="{{url('admin/home-slider')}}/{{$test->slug}}" class="dropdown-item">Edit</a>

                                                <a href="{{route('admin.HomeSlider.status',$test->id)}}" class="dropdown-item">
                                                    @if($test->status == '1')
                                                        In-active
                                                    @else
                                                        Active
                                                ...
                                                <a onclick="return confirm('Are you sure you want to delete this drh activity?')" href="{{url('admin/home-slider/delete')}}/{{$test->id}}" class="dropdown-item">Delete</a>
                                            </div>
                                        </div>
                                    </td>
                                </tr>
